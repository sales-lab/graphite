...
This same steps can be used to access the Reactome, KEGG, SPIKE and NCI databases (through the \Robject{reactome}, \Robject{kegg}, \Robject{spike} and \Robject{nci} lists, respectively).
...
In \Biocpkg{graphite}, \Biocpkg{DEGraph} has two dedicated functions. The first is \Rfunction{runDEGraph}, which performs the analysis on a single pathway.
...
The second function is \Rfunction{runDEGraphMulti}, which easily performs the analysis on the entire pathway database having as result a list of two elements: a list with the results of the pathway analyses and the list of generated errors.
...
In \Biocpkg{graphite}, \Biocpkg{topologyGSA} has two dedicated functions. The first is \Rfunction{runTopologyGSA}, which performs the analysis on a single pathway.
...
The second function is \Rfunction{runTopologyGSAMulti}, which easily performs the analysis on the entire pathway database having as result a list of two elements: a list with the results of the pathway analyses and the list of generated errors.
...
In \Biocpkg{graphite}, \Biocpkg{clipper} has two dedicated functions. The first is \Rfunction{runClipper}, which performs the analysis on a single pathway.
